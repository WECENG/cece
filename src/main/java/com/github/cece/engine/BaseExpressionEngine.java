package com.github.cece.engine;
...
import com.github.cece.operator.Operator;
import com.github.cece.operator.OperatorConstant;
import com.github.cece.operator.OperatorContext;
import com.github.cece.operator.OperatorManager;
import com.github.cece.utils.ContextVarUtil;
import lombok.*;
...
public abstract class BaseExpressionEngine<N> {
    ...
    public <R> List<R> evaluates(String expression, Function<String, VarMeta> varExprFunction, boolean verify, BiPredicate<Object, String> valueFilter, UnaryOperator<String> contextVarFunction) {
        ...
        for (AtomicInteger i = new AtomicInteger(); i.get() < expression.length(); i.getAndIncrement()) {
            ...
            if (Character.isDigit(ch) || ch == OperatorConstant.DOT ||
                    (ch == OperatorConstant.NEG && Character.isDigit(expression.charAt(i.get() + 1)))) {
            ...
            } else if (ch == OperatorConstant.LEFT_CONTEXT) {
    ...
    private int parseNumber(String expression, int index, Deque<Object> values) {
        ...
        while (index < expression.length() && (Character.isDigit(expression.charAt(index)) ||
                expression.charAt(index) == OperatorConstant.DOT || expression.charAt(index) == OperatorConstant.NEG)) {
    ...
    public int parseVariable(String expression, int startIndex, LinkedList<Object> values,
        ...
        Pattern pattern = Pattern.compile(OperatorConstant.WORD_VARIABLE_REGEX);
    ...
    public static Object extractValue(Object context, VarMeta varMeta, BiPredicate<Object, String> valueFilter) {
        ...
        List<String> keys = Arrays.asList(varName.split(OperatorConstant.ESCAPED_DOT));
     ...
     * @param nullable 可空
     * @return 展开元素
     * @implSpec 如果元素为null，且参数nullable为false,
